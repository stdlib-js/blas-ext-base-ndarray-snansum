...
var snansum = tryRequire( resolve( __dirname, './../lib/native.js' ) );
var opts = {
...
tape( 'the function has an arity of 1', opts, function test( t ) {
	t.strictEqual( snansum.length, 1, 'has expected arity' );
	t.end();
});

tape( 'the function computes the sum of a one-dimensional ndarray, ignoring `NaN` values', opts, function test( t ) {
	var x;
	var v;

	x = new Float32Array( [ 1.0, -2.0, -4.0, 5.0, NaN, 0.0, 3.0, 0.0, -3.0, 3.0 ] );
	v = snansum( [ vector( x, 10, 1, 0 ) ] );
	t.strictEqual( v, 3.0, 'returns expected value' );

	x = new Float32Array( [ 1.0, -2.0, -4.0, NaN, 5.0, 0.0, 3.0 ] );
	v = snansum( [ vector( x, 7, 1, 0 ) ] );
	t.strictEqual( v, 3.0, 'returns expected value' );

	x = new Float32Array( [ -4.0, -5.0 ] );
	v = snansum( [ vector( x, 2, 1, 0 ) ] );
